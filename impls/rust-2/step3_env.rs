...
fn eval(val: MalType, env: &mut Env) -> MalType {
    match val {
        MalType::Symbol(s) if s != DEF && s != LET => {
        ...
        MalType::List(xs) => {
            ...
            match op {
                ...
                MalType::Symbol(s) if s == "def!" => {
                    if xs.len() != 3 {
                        return types::invalid_parameter_length_error(xs.len(), 3);
                    }
                    let MalType::Symbol(ref s) = xs[1] else {
                        return types::invalid_argument_error();
                    };
                    let v = eval(xs[2].clone(), env);
                    match v {
                        MalType::Error(_) => (),
                ...
                MalType::Symbol(s) if s == "let*" => {
                    ...
                    match new_env {
                        Ok(mut env) => eval(xs[2].clone(), &mut env),
                        Err(e) => e,
                    }
                },
