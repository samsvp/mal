            ...
            .list => |*list| {
                ...
                if (items[0] == .symbol) {
                    ...
                    if (std.mem.eql(u8, s_chars, "defmacro!")) {
                        ...
                        if (items[1] != .symbol) {
                            return MalType.makeError(allocator, "Second parameter of 'def!' must be a symbol");
                        }
                        ...
                        switch (val) {
                            ...
                            .function => {},
                            else => return MalType.makeError(allocator, "'defmacro!' value must be a function."),
                        }

                        val.function.setMacro();
                ...
                switch (fst) {
                    ...
                    .function => |*f| {
                        if (f.isMacro()) {
                            ...
                            var val = eval(allocator, &ast, fn_env);
                            defer val.deinit(allocator) catch unreachable;
                            swapS(allocator, &s, &val);
                            continue;
                        }
...
pub fn main() !void {
    ...
    const def_not: []const u8 =
        \\(def! not (fn* (a) (if a false true)))
    ;
    allocator.free(try rep(allocator, def_not, env));
    const load_file =
